...
INFO_PAGES = [
    (
        ...
        "Спочатку реєстрація й оплата участі — після цього відкривається перша кімната. Далі лише вперед: друга, третя, четверта й п’ята відкриваються строго по черзі.\n"
        "Пропустити кімнату або зайти «з середини» не можна: наступна стане доступною лише після правильного ключового слова.\n\n"
        "Що є відповіддю?\n"
        ...
        "Сначала регистрация и оплата участия — после этого открывается первая комната. Дальше только вперёд: вторая, третья, четвёртая и пятая открываются строго по порядку.\n"
        "Пропустить комнату или зайти «с середины» нельзя: следующая станет доступна только после верного ключевого слова.\n\n"
